...
export async function uploadObjectToS3({ key, body, contentType = 'application/octet-stream', cacheControl }) {
  if (!s3Client || !AWS_S3_BUCKET) {
  ...
  return { bucket: AWS_S3_BUCKET, key };
}
...
export async function persistLivestreamClip(clip) {
  const client = ensureClient();
  if (!client) return;

  const row = {
    mint_id: clip.mintId,
    captured_by: clip.capturedBy ?? null,
    label: clip.label ?? null,
    started_at: clip.startedAt,
    ended_at: clip.endedAt,
    duration_ms: clip.durationMs,
    viewer_count_min: clip.viewerCountMin ?? null,
    viewer_count_max: clip.viewerCountMax ?? null,
    params: clip.params ?? null,
    s3_audio_key: clip.s3AudioKey ?? null,
    s3_video_key: clip.s3VideoKey ?? null,
    status: clip.status ?? 'ready',
    notes: clip.notes ?? null,
  };

  const { error } = await client.from('livestream_clips').insert(row);
  if (error) {
    console.error('[supabase] Failed to insert livestream clip', error.message);
  }
}

export async function persistPlatformMinuteMetrics({ fetchedAt, entries = [] }) {
  const client = ensureClient();
  if (!client) return;
  try {
    const ts = typeof fetchedAt === 'string' ? new Date(fetchedAt).getTime() : Number(fetchedAt);
    ...
    const liveStreams = Array.isArray(entries) ? entries.length : 0;
    let totalViewers = 0;
    let totalMarketCap = 0;
    for (const item of entries) {
      const v = Number(item?.num_participants ?? item?.numParticipants ?? 0);
      if (Number.isFinite(v)) totalViewers += v;
      const mc = Number(item?.market_cap ?? 0);
      if (Number.isFinite(mc)) totalMarketCap += mc;
    }
    ...
    const { error } = await client
      ...
      .upsert(row, { onConflict: 'bucket' });
    if (error) {
      console.error('[supabase] Failed to upsert platform_metrics_minute', error.message);
    }
  } catch (error) {
    console.error('[supabase] persistPlatformMinuteMetrics failed:', error?.message ?? error);
  }
}
