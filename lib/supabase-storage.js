...
export async function persistLiveRoster(entries = [], fetchedAt = new Date().toISOString()) {
  ...
  const liveMintIds = latestRows.map((row) => row.mint_id);
  if (liveMintIds.length) {
    const inList = `(${liveMintIds.map((id) => `"${id}"`).join(',')})`;
    const { error: markInactiveError } = await client
      .from('livestream_latest')
      .update({ is_live: false })
      .not('mint_id', 'in', inList)
      .eq('is_live', true);
    if (markInactiveError) {
      console.error('[supabase] Failed to mark inactive livestreams', markInactiveError.message);
    }
  }
}
